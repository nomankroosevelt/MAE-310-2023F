...
for inel = 2 : 2
    ...
    for e = 1 : n_el
        ...
        x_ele(4) = x_coor(IEN(4,e));
        
        for l = 1 : n_int
            dx_dxi = 0.0;
            x_l = 0.0;
            for a = 1 : 4
                dx_dxi = dx_dxi + x_ele(a) * CubicShape(a, xi(l),1);
                x_l = x_l +x_ele(a) * CubicShape(a,xi(l),0);
            end
            dxi_dx = 1/dx_dxi;
            
            for a = 1 : 4
                for b = 1 : 4
                    k_e(a,b) = k_e(a,b) + weight(l) * CubicShape(a,xi(l),1) * CubicShape(b,xi(l),1) * dxi_dx;
                end
            end
            
            for a = 1 : 4
                f_e(a) = f_e(a) + weight(l) * CubicShape(a,xi(l),0) * f(x_l) * dx_dxi;
            end 
        end
    ...
    for e = 1 : n_el
        for l = 1 : n_int
            ...
            dx_dxi = 0.0;
            for a = 1 : 4
                dx_dxi = dx_dxi + x_coor(IEN(a,e)) * CubicShape(a, xi(l), 1);
            end
            
            for a = 1 : 4
                uh_dx = uh_dx + disp(IEN(a,e)) * CubicShape(a, xi(l), 1)/dx_dxi;
                uh = uh + disp(IEN(a,e)) * CubicShape(a, xi(l), 0);
                xl = xl + x_coor(IEN(a,e)) * CubicShape(a, xi(l), 0);
            end
